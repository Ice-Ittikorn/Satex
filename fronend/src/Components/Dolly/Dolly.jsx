    ...
    <div className="dolly-container">
      ...
      {Object.keys(groupedOrders).length === 0 ? (
        ...
        Object.keys(groupedOrders).map((tableId) => {
          const ordersForTable = groupedOrders[tableId];
          const uniqueStatuses = [...new Set(ordersForTable.map(order => order.status))];
            ...
            <div key={tableId} className="dolly-table">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {uniqueStatuses.length > 0 && (
                  <span className={getStatusClass(uniqueStatuses[0])}>
                    {uniqueStatuses.join(', ')}
                  </span>
                )}
                <button 
